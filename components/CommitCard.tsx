...
export function CommitCard({ id, title, participants, stakeAmount, stakeToken, timeRemaining }: CommitCardProps) {
	return (
		<Link href={`/commit/${id}`}>
			<div className="bg-[#DCDCDC] dark:bg-[#2A2A2A] rounded-xl p-6 hover:shadow-md transition-shadow duration-200">
				...
				<div className="flex items-center justify-between mb-4">
					...
					<div className="text-gray-600 dark:text-gray-400">
						<span className="text-sm font-medium">{stakeAmount} {stakeToken} stake</span>
					</div>
				</div>
